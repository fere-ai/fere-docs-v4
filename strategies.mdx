...
<Tip>
  **Context That Compounds.** Fere distills past runs into the signals that actually matter — not a wall of logs, but a smart research notebook that makes every cycle more informed than the last. This applies to **agent-mode** strategies (the LLM reasons each tick). **Script-mode** strategies run a pinned workspace file as written each cycle — fast and deterministic, without feeding prior-run LLM history into that tick.

  **When do you need to fund a strategy wallet?** Trading strategies (Hyperliquid, Polymarket, or instructions that place buys/sells/orders) get an **isolated strategy wallet** and stay in setup until you fund it and activate. **Research, summaries, and read-only scanners** — including script-mode alerts that only report — stay on your main wallet and go live without that funding checklist. Script mode alone does not mean "fund a new wallet."
</Tip>
...
<Info>
  **Durable trading bots use builder → script → schedule.**

  For an ongoing Polymarket / Hyperliquid / on-chain bot you want to run on a timer, ask Fere to **create / build the strategy** first so `strategy_builder` writes and verifies a script under `/work/scripts`. Then schedule it in **script mode** with that entrypoint.

  Saying only *"schedule this"* without a saved script can land as **agent mode** (LLM each tick). One-off research or market summaries can stay agent mode. If script validation fails, schedule creation is **rejected** — Fere will not silently fall back to agent mode.
</Info>
...
**Every hour, Fere will:** scan the chain for candidates, run social and TA analysis on each, score them, and only execute a trade if the quality bar is met. Open the strategy detail page after a run to read the executive briefing, then use the right rail for **Positions**, **Transactions**, **Runs**, and **Files**.
...
### Right sidebar — wallet, last run, and tabs

On desktop, a compact **320px rail** on the right stays visible while you are on a strategy detail page. If you open the wallet drawer, the rail shifts left — it is never hidden behind the wallet.

Strategy wallets also appear on **[Portfolio](/portfolio)** so you can inspect holdings, trades, and open orders for that strategy without opening its chat.

From top to bottom:

| Area | What you see |
| --- | --- |
| **Status bar** | How many times the strategy has run, **Live** or **Paused**, and **Pause** / **Resume**. |
| **Wallet row** | Spendable balance (or *Balance unavailable*), **Open portfolio**, **Fund**, and the strategy wallet cluster. |
| **Last run** | Outcome icon, the label **Last run**, and a relative timestamp. Click it to open that run. |
| **Tabs** | **Positions**, **Transactions**, **Runs**, and **Files**. |

| Tab | What you see |
| --- | --- |
| **Positions** | Open holdings for this strategy wallet (same rows as portfolio). Swap **Sell** is hidden — unwind ordinary tokens with **Withdraw to Main**. **Redeem** stays for resolved Polymarket shares, and **Sell** stays on Hyperliquid spot rows, because Withdraw cannot move those until they are converted to cash. Empty and load-error copy match the wallet overlay. |
| **Transactions** | Swaps, buys, sells, and other activity for this strategy. Scroll for more; tap a row for details. |
| **Runs** | Numbered scheduled cycles (`Run #1`, `Run #2`, …) with timestamps. Click a row to open **Run details** (full outcome and activities from that cycle). |
| **Files** | Files the agent generated — reports, scripts, exports. Click to preview or download. Copied or shared strategies may show that files are not available to your account. |

If holdings fail to load, the Positions tab shows *Could not load holdings* with a **Refresh** action. Other tabs show a retry state if their feed cannot load.

On mobile, tap the floating **Strategy** button to open a bottom sheet with the same tabs.
...
<Info>
  **Pause keeps funds in the strategy wallet.** Pausing stops new runs without sweeping capital. Use **Withdraw** in the wallet panel (or delete the strategy) to move spot tokens back to your main wallet. Closed strategies that still hold a balance appear under **Archived strategies** until emptied — see [Wallet](/wallet#strategy-wallets).
</Info>
