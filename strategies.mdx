...
<Info>
  This catalog lists **85** sandbox-allowed tools (28 write money or on-chain/Polymarket/perp state). **Examples** appear only for wallet on-chain, Polymarket, and Hyperliquid perp tools (52 tools). **Scheduling** (`create_scheduled_task`, `modify_scheduled_task`, `cancel_scheduled_task`) is **not** in the sandbox — create or change schedules from Strategy chat or the UI, not from `fere_tools`.
</Info>
...
<Tip>
  **Async writes:** Swaps, transfers, limit orders, Polymarket orders, and perp opens return `task_id`(s). Confirm with `F.wait_for_tasks(task_ids)` — a client-side loop over `F.get_task_status` (~3s interval, ~180s cap under the sandbox run timeout). Legacy `poll_transaction_status(task_ids=[...], tool_name="...")` still works but holds one long gateway connection; prefer `wait_for_tasks` for new scripts.
</Tip>
...
```text
Get token holders for a given EVM contract address.

Returns the top holders of a token with their balances and percentages.
Useful for analyzing token distribution and whale concentration on EVM chains.

**Not supported:** ``solana`` (use ``token_risk_analysis_tool`` for Solana
holder concentration via GoPlus).

Args:
    chain: EVM network slug (eth, base, polygon, arbitrum, bsc, etc.).
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `chain` | `any` | Yes | typing.Annotated[str, 'EVM blockchain supported by Ankr (e.g. eth, base, polygon, arbitrum). Not Solana — for Solana holder concentration use token_risk_analysis_tool.'] |
...
```text
Read one wallet task's status once (non-blocking - no polling loop).

Unlike ``poll_transaction_status`` (which blocks server-side until the task
reaches a terminal state), this returns the current state immediately so the
caller can poll on its own cadence::

    {"task_id": ..., "status": ..., "result": ..., "task_found": bool}

``status`` is the Celery task state - PENDING / STARTED / RETRY while still
running, or SUCCESS / FAILURE / REVOKED once terminal. An unknown or
not-yet-claimed task_id comes back as ``status="PENDING"`` with
``task_found=False`` (that is not an error). ``result`` carries the wallet
payload on terminal states (txn details, error/error_code, and any
Polymarket fills / take-profit ids merged from the action output).
```
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `task_id` | `string` | Yes | Wallet-service task_id returned by a write tool (e.g. trade_tokens, transfer_tokens, create_limit_order, polymarket_* or perp writes). Reads the current status once and returns immediately. |
...
st = F.get_task_status("<task_id_from_prior_write>")
print(st.get("status"), st.get("task_found"))
```
...
| `timeout_seconds` | `integer` | No | Maximum seconds to block waiting for a terminal state (default kept under the ~100s edge/gateway timeout so a real result returns before the edge cuts the connection). |
...
```text
Return supported source (wallet) and destination (DeBridge) chains.
```
...
if resp.get("task_ids"):
    rows = F.wait_for_tasks(resp["task_ids"])
    print(rows)
```
...
if resp.get("task_id"):
    print(F.wait_for_tasks([resp["task_id"]]))
```
...
BUY, shares for SELL). A $5 min-notional is enforced on BUY orders
only; SELL orders are not pre-checked on our side, though Polymarket's
CLOB still rejects tiny SELL notional (shares × price).
``take_profit_price`` on BUY only schedules a
