...
| --- | --- |
| **Past Runs** | Numbered list of every scheduled cycle (`Run #1`, `Run #2`, …) with timestamps. Click a run to open a detail modal with the full agent outcome and any on-chain **Activities** from that cycle. For **Day Trading** with multiple markets, click a market card on the settings page to select it — Past Runs then shows that market only (status bar chip names the scoped market). |
| **Transactions** | Swaps, buys, sells, and other trades this strategy executed. Tap a row to expand full details inline. For **Day Trading**, Transactions follow the same selected market as Past Runs (not a cross-market aggregate). |
